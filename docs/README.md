...
```bash
# NixOS operations
make build HOST=master          # Test configuration
make switch HOST=master         # Apply configuration
make switch HOST=worker-1       # Deploy to worker

# Kubernetes operations  
make flux-check           # Check Flux status
...
make sops-edit FILE=kubernetes/.../secret.sops.yaml        # Edit K8s secret
```
